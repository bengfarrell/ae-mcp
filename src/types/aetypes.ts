import type { File, Folder } from './primitives';
import type {
    AutoOrientType,
    BlendingMode,
    FrameBlendingType,
 ...
 * 
 * @returns {number} A floating-point, pseudo-random number in the range [0..1].
 */
 ...
 * @returns {string} The string name of the Enum value.
 */
declare var getEnumAsString: (enumValue: number) => string

/**
 * Converts a formatted string for a frame time value to a number of seconds, given a specified frame rate.
 * 
 * @param {string} formattedTime - The frame time value, a string specifying a number of frames in the project's current time display format.
 * @param {number} fps - The frames-per-second, a floating-point value.
 * @param {boolean} [isDuration=false] - Optional. When true, the time is a duration (measured from frame 0). When false, the time is measured from the project's starting frame.
 * @returns {number} The number of seconds as a floating-point value.
 */
...
/**
 * Converts a numeric time value (a number of seconds) to a frame time value; that is, a formatted string that shows which frame corresponds to that time, at the specified rate.
 * 
 * @param {number} time - The number of seconds, a floating-point value.
 * @param {number} fps - The frames-per-second, a floating-point value.
 * @param {boolean} [isDuration=false] - Optional. When true, the time is a duration (measured from frame 0). When false, the time is measured from the project's starting frame.
 * @returns {string} A string in the project's current time display format.
 */
 ...
 * 
 * @param {string} text - The string to display. Truncated if too long for the Info panel.
 * @returns {void} Nothing.
 */
...
/**
 * Determines if the specified After Effects object still exists.
 * 
 * @param {object} obj - The After Effects object to check for validity.
...
declare class Application {
     ...
     * 
     * @returns {Project | null} A new Project object, or null if a new project cannot be created.
     */
     ...
     * 
     * @param {File} [file] - Optional. An ExtendScript File object specifying the project file to open. If not supplied, the method opens an Open Project dialog box.
     * @returns {Project | null} A new Project object for the specified project, or null if the user cancels the Open dialog box or the specified file cannot be opened.
     */
     ...
     * 
     * @param {Folder} folder_object_to_watch - An ExtendScript Folder object specifying the folder to watch.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {boolean} pause - When true, pauses a current watch-folder process. When false, resumes a paused watch-folder process.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {PurgeTarget} target - The type of cached information to purge. One of: ALLCACHES, UNDO, IMAGE_CACHES, CONFORMED_MEDIA, or SNAPSHOT.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {string} undoString - A string describing the action. The string appears in the Undo/Redo menu.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {boolean} alert - When true, displays an alert dialog box when dialog box suppression ends. When false, does not display an alert.
     * @returns {void} Nothing.
     */
     ...
     * The value of n is: 2 GB for 32-bit Windows, 4 GB for 64-bit Windows, 3.5 GB for Mac OS.
     * 
     * @param {number} imageCachePercentage - The percentage of memory assigned to image cache.
     * @param {number} maximumMemoryPercentage - The maximum usable percentage of memory.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {boolean} doSave - When true, preferences are saved when the application is quit. When false, preferences are not saved.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {string} stringToExecute - A string containing JavaScript code to be executed.
     * @param {number} delay - A number of milliseconds to wait before executing the JavaScript.
     * @param {boolean} repeat - When true, executes the script repeatedly with the specified delay between each execution. When false, the script is executed only once.
     * @returns {number} A unique identifier for this task, which can be used to cancel it with app.cancelTask().
     */
     ...
     * 
     * @param {number} taskID - A number assigned to the task when it was scheduled with app.scheduleTask().
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {File} file - An ExtendScript File object referencing the ASE file to load.
     * @returns {_Swatch} A _Swatch object containing the color information from the ASE file.
     */
    parseSwatchFile(file: File): _Swatch

    /**
     * Finds the menu command ID for a given menu command string.
     * 
     * @param {string} str - The menu command string to search for.
     * @returns {number} The menu command ID, or 0 if the command is not found.
     */
    findMenuCommandId(str: string): number

    /**
     * Executes a menu command with the given command ID.
     * 
     * @param {number} id - The menu command ID to execute.
     * @returns {void} Nothing.
     */
    executeCommand(id: number): void

    /**
     * Gets an environment variable value (CC2015 and earlier).
     * 
    ...
    /**
     * Schedules a function to be executed after a specified delay (CC2015 and earlier).
     * 
     * @param {() => void} func - The function to execute.
     * @param {number} [delay] - Optional. The delay in milliseconds before executing the function.
     * @returns {number} A unique identifier for the timeout, which can be used to cancel it.
     */
    setTimeout(func: () => void, delay?: number): number
    
    /**
     * Cancels a timeout that was scheduled with setTimeout (CC2015 and earlier).
     * 
     * @param {number} id - The timeout identifier returned by setTimeout.
     * @returns {void} Nothing.
     */
...
declare class AVItem extends Item {
    ...
    /**
     * The frame duration for the item in seconds.
     */
    frameDuration: number

    /**
     * The total duration of the item in seconds.
     */
    duration: number
    ...
    /**
     * Current time of the item in seconds.
     */
    time: number
     ...
     * 
     * @param {File} file - An ExtendScript File object for the proxy file.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {File} file - An ExtendScript File object for the first file in the sequence.
     * @param {boolean} forceAlphabetical - When true, forces alphabetical order for the sequence.
     * @returns {void} Nothing.
     */
     ...
     * 
     * @param {ThreeDColorValue} color - The color of the solid as a three-element array of RGB values in the range [0.0, 1.0].
     * @param {string} name - The name of the solid.
     ...
     * @param {number} width - The width of the placeholder in pixels.
     * @param {number} height - The height of the placeholder in pixels.
     * @param {number} frameRate - The frame rate of the placeholder in frames per second.
     * @param {number} duration - The duration of the placeholder in seconds.
     * @returns {void} Nothing.
     */
...
declare class CompItem extends AVItem {
    ...
    /**
     * The background color of the composition as a three-element array of RGB values in the range [0.0, 1.0].
     */
     ...
     * 
     * @returns {CompItem} A new CompItem that is a duplicate of this composition.
     */
    duplicate(): CompItem
     ...
     * 
     * @param {number} index - The index of the layer to retrieve (1-based).
     * @returns {Layer} The layer at the specified index.
     */
    layer(index: number): Layer
    /**
     * Gets a layer from this composition relative to another layer.
     * 
     * @param {Layer} otherLayer - The reference layer.
     * @param {number} relIndex - The relative index from the reference layer.
     * @returns {Layer} The layer at the relative position.
     */
    layer(otherLayer: Layer, relIndex: number): Layer
    /**
     * Gets a layer from this composition by name.
     * 
     * @param {string} name - The name of the layer to retrieve.
     * @returns {Layer} The layer with the specified name.
     */
     ...
     * 
     * @returns {Viewer | null} The Viewer object for the Composition panel, or null if the composition could not be opened.
     */
     ...
     * 
     * @param {number} time - The time in seconds of the frame to save.
     * @param {File} file - An ExtendScript File object specifying where to save the PNG file.
     * @returns {void} Nothing.
     */
